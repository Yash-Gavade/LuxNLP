...
def main():
    ...
    # These are kept for CLI compatibility but NOT required for old transformers
    ...
    # Many older transformers versions do NOT support evaluation_strategy/save_strategy.
    # We keep args minimal and do evaluation manually with trainer.evaluate().
    ta_kwargs = dict(
    ...
    # Some older versions may not support report_to either, so fallback again if needed
    try:
